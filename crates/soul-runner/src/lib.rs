...
use soul_core::{
    App, Ctx, Event, HardButton, KeyCode, APP_HEIGHT, SCREEN_WIDTH,
    SYSTEM_STRIP_H,
};
        ...
        match &desc.kind {
            AppKind::Scripted { script, db } => {
                ...
                match ScriptedApp::new(script_stem, &script_src, soul_db) {
                    Ok(app) => {
                        log::info!("App {} ({}) loaded", app.declared_name().as_deref().unwrap_or(script_stem), script_stem);
                        AppSlot::Scripted {
                            app: Box::new(app),
                            db_path,
                        }
                    ...
                    Err(e) => {
                        ...
                        let err_script = format!(
                            "let app_id = \"error.{}\"; let app_name = \"{}\"; fn on_draw() {{ title_bar(\"{}\"); label(10, 80, \"Script error.\"); }} fn on_event(_ev) {{}}",
                             script_stem, script_stem, script_stem
                        );
