...
use rhai::{Engine, Scope, AST, Dynamic, Map, Position, EvalAltResult};
use soul_core::{App, Ctx, Event, SCREEN_WIDTH, APP_HEIGHT};
use soul_db::Database;
use soul_ui::Form;
use soul_ui::TextInput;
use soul_ui::{TextArea, Keyboard, TypedKey, KEYBOARD_HEIGHT};
...
impl ScriptedApp {
    pub fn new(name: &str, script: &str, db: Database) -> Result<Self, Box<rhai::ParseError>> {
        ...
        engine.register_fn("new_text_input", |x: i32, y: i32, w: i32, h: i32| {
            TextInput::new(Rectangle::new(Point::new(x, y), Size::new(w as u32, h as u32)))
        });
        engine.register_fn("get_text", |input: &mut TextInput| input.text().to_string());
        engine.register_fn("set_text", |input: &mut TextInput, text: String| { input.set_text(text); });
        engine.register_fn("insert_char", |input: &mut TextInput, c: String| {
            if let Some(ch) = c.chars().next() {
                let _ = input.insert_char(ch);
            }
        });
        engine.register_fn("backspace", |input: &mut TextInput| { let _ = input.backspace(); });
        ...
        engine.register_fn("new_text_area", |x: i32, y: i32, w: i32, h: i32| {
            TextArea::new(Rectangle::new(Point::new(x, y), Size::new(w as u32, h as u32)))
        });
        engine.register_fn("get_text", |area: &mut TextArea| area.text().to_string());
        engine.register_fn("set_text", |area: &mut TextArea, text: String| { area.set_text(text); });
        engine.register_fn("insert_char", |area: &mut TextArea, c: String| {
            if let Some(ch) = c.chars().next() {
                let _ = area.insert_char(ch);
            }
        });
        engine.register_fn("backspace", |area: &mut TextArea| { let _ = area.backspace(); });
        engine.register_fn("enter", |area: &mut TextArea| { let _ = area.enter(); });
        engine.register_fn("pen_down", |area: &mut TextArea, x: i32, y: i32, now_ms: i32| { area.pen_down(x as i16, y as i16, now_ms as u64); });
        engine.register_fn("pen_move", |area: &mut TextArea, x: i32, y: i32| { area.pen_moved(x as i16, y as i16); });
        engine.register_fn("pen_up", |area: &mut TextArea, x: i32, y: i32| { area.pen_released(x as i16, y as i16); });
        ...
        engine.register_fn("handle_pen", |kb: &mut Keyboard, x: i32, y: i32, down: bool| -> String {
            let res = if down {
        ...
        engine.register_fn("from_json", |json: String| Form::from_json(&json).unwrap_or_else(|| Form::new("error")));
        engine.register_fn("to_json", |form: &mut Form| form.to_json());

        // Register Database methods (Global)
        engine.register_fn("db_insert", |category: i32, data: Vec<u8>| {
            unsafe { ACTIVE_DB.map(|db| (*db).insert(category as u8, data) as i32).unwrap_or(0) }
        });
        engine.register_fn("db_get_data", |id: i32| -> Vec<u8> {
            unsafe { ACTIVE_DB.and_then(|db| (*db).get(id as u32).map(|r| r.data.clone())).unwrap_or_default() }
        });
        engine.register_fn("db_get_ids_in_category", |category: i32| -> rhai::Array {
            unsafe {
                ACTIVE_DB.map(|db| (*db).iter_category(category as u8).map(|r| Dynamic::from(r.id as i32)).collect()).unwrap_or_default()
            }
        });
        engine.register_fn("db_update", |id: i32, data: Vec<u8>| {
            unsafe { if let Some(db) = ACTIVE_DB { (*db).update(id as u32, data); } }
        });
        engine.register_fn("db_delete", |id: i32| {
            unsafe { if let Some(db) = ACTIVE_DB { (*db).delete(id as u32); } }
        });
        ...
        engine.register_fn("to_bytes", |s: String| {
            s.into_bytes()
        });

        // Register Context methods
        engine.register_fn("invalidate", |x: i32, y: i32, w: i32, h: i32| {
            unsafe {
                if let Some(ctx_ptr) = ACTIVE_CTX {
                    let ctx = &mut *(ctx_ptr as *mut Ctx);
                    ctx.invalidate(Rectangle::new(Point::new(x, y), Size::new(w as u32, h as u32)));
                }
            }
        });
        engine.register_fn("invalidate_all", || {
            unsafe { if let Some(ctx_ptr) = ACTIVE_CTX { let ctx = &mut *(ctx_ptr as *mut Ctx); ctx.invalidate_all(); } }
        });

        // Register Global drawing functions
        engine.register_fn("title_bar", |title: String| {
            unsafe { if let Some(canvas) = ACTIVE_CANVAS { (*canvas).title_bar(&title); } }
        });
        engine.register_fn("button", |x: i32, y: i32, w: i32, h: i32, label: String, pressed: bool| {
            unsafe { if let Some(canvas) = ACTIVE_CANVAS { (*canvas).button(x, y, w as u32, h as u32, &label, pressed); } }
        });
        engine.register_fn("label", |x: i32, y: i32, text: String| {
            unsafe { if let Some(canvas) = ACTIVE_CANVAS { (*canvas).label(x, y, &text); } }
        });
        engine.register_fn("clear", || {
            unsafe { if let Some(canvas) = ACTIVE_CANVAS { (*canvas).clear(); } }
        });
        engine.register_fn("draw_form", |form: Form| {
            unsafe { if let Some(canvas) = ACTIVE_CANVAS { (*canvas).draw_form(&form); } }
        });
        engine.register_fn("draw_text_input", |input: TextInput| {
            unsafe { if let Some(canvas) = ACTIVE_CANVAS { (*canvas).draw_text_input(&input); } }
        });
        engine.register_fn("draw_text_area", |area: TextArea| {
            unsafe { if let Some(canvas) = ACTIVE_CANVAS { (*canvas).draw_text_area(&area); } }
        });
        engine.register_fn("draw_keyboard", |kb: Keyboard| {
            unsafe { if let Some(canvas) = ACTIVE_CANVAS { (*canvas).draw_keyboard(&kb); } }
        });
        engine.register_fn("draw_rect", |x: i32, y: i32, w: i32, h: i32, color: i32| {
            unsafe { if let Some(canvas) = ACTIVE_CANVAS { (*canvas).rect(x, y, w as u32, h as u32, color as u8); } }
        });
...
impl App for ScriptedApp {
    fn handle(&mut self, event: Event, ctx: &mut Ctx<'_>) {
        let mut map = Map::new();
        match event {
            Event::AppStart => { map.insert("type".into(), "AppStart".into()); }
            Event::AppStop => { map.insert("type".into(), "AppStop".into()); }
            Event::PenDown { x, y } => {
            ...
            _ => { map.insert("type".into(), "Other".into()); }
        }
        ...
        unsafe {
            ...
            if let Err(e) = self.engine.call_fn::<()>(&mut self.scope, &self.ast, "on_event", (map,)) {
                self.last_error = Some(ScriptError::from_rhai_error(&self.script_name, "on_event", &e));
            }
        ...
        D: DrawTarget<Color = Gray8>,
    {
        unsafe {
            ...
            let erased = core::mem::transmute::<&mut dyn ObjectSafeDraw, *mut dyn ObjectSafeDraw>(bridge);
            ACTIVE_CANVAS = Some(erased);
            ACTIVE_DB = Some(&mut self.db as *mut Database);
            
            // Execute on_draw and capture any errors for std environments to log
            if let Err(e) = self.engine.call_fn::<()>(&mut self.scope, &self.ast, "on_draw", ()) {
                self.last_error = Some(ScriptError::from_rhai_error(&self.script_name, "on_draw", &e));
            }
